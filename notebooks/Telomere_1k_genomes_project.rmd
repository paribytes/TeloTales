...
telo_data <- read.csv("data/processed/1k_genome_project_analysis_csv.csv", header = TRUE, row.names = NULL)
head(telo_data) # Inspect dataset structure
...
#If the row has missing data for ScaledTelomericReads, then the code below will remove that row in the updated "telo_data" dataframe.
...
```{r SubsetDataframes}
#Create a subset containing superpopulation and telomere content to a new dataframe - "superpop_data"
superpop_data <- telo_data[, c("Superpopulation.name", "ScaledTelomericReads")]

# Remove leading/trailing whitespace in population labels
telo_data$Superpopulation.name <- trimws(telo_data$Superpopulation.name)

# Listing unique values in the Superpopulation.name column to ensure all is well (should be getting 5 superpopulations)
