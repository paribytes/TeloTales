...
#without_5 <- forward_data %>% filter(chromosome != "chr5")
...
```{r ExtractingValuesOfPotentialOutliers}


#boxplot.stats(forward_data$telomere.length.in.kbps)$out
...
#identify_outliers(
#  data = forward_data,
#  variable = "telomere motif in kbps"
#s)
...
lower_bound <- quantile(final_across$telomere.length.in.kbps, 0.01)
lower_bound

#So that means all the observations below zero and above 3.342 will be considered as potential outliers
```


```{r PercentilesMethod}

upper_bound <- quantile(final_across$telomere.length.in.kbps, 0.99)
upper_bound
...
```{r ExcludingOutliers}
# Assuming `forward_data` is the dataset and `telomere length in kbps` is where the outliers are.
#outliers_removed <- subset(forward_data, telomere.length.in.kbps <= upper_bound & telomere.length.in.kbps >= lower_bound)
...
#outliers_removed$telomere.length.in.kbps <- as.numeric(outliers_removed$telomere.length.in.kbps)

#without_5$telomere.length.in.kbps <- as.numeric(without_5$telomere.length.in.kbps)
...
```{r Outliersdetection}
#summary(forward_data$telomere.length.in.kbps)
