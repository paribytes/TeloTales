...
ggplot() +
  ...
  labs(title = "Average Telomeric Reads within Twenty-Six global human Populations", x = "Populations", y = "Average Telomeric Reads") +
  theme_minimal() +
  scale_fill_manual(values = color_mapping) +
  guides(fill = guide_legend(title = "Populations", override.aes = list(fill = population_colors))) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 8),  # Adjust the font size for x-axis labels, tilted the labels to 45 degrees 
...
#Following tests were done to get the overview of the distribution and variability of telomeric reads
...
```{r Normality Test}
#Normality test was done to see if the data follows a normal distribution using tests like Shapiro-Wilk test or Kolmogorov-Smirnov test.

#Since Shapiro-Wilk is suitable for smaller sample size and we are working with larger sample size, oped for Kolmogorov-Smirnov test
...
```{r Non-ParametricANOVA }
#Since there are significant differences in telomeric reads among the populations and the assumptions are not met, considered using a non-parametric alternative like the Kruskal-Wallis test.

# Kruskal-Wallis test (non-parametric alternative)
kruskal.test(ScaledTelomericReads ~ Population, data = telo_data)

#df: degrees of freedom, which is the number of populations minus one aka here 25.
#the p-value is less then 0.05 which indicates that there are significant differences between the populations. 
```
...
```{r Post-hoc tests}
#the Kruskal-Wallis test indicates significant differences, therefore performed post-hoc pairwise comparisons using the pairwise.wilcox.test function:

pairwise.wilcox.test(telo_data$ScaledTelomericReads, telo_data$Population, p.adjust.method = "bonferroni")

```



```{r Normality Test for comparison among superpopulations}

#Normality test was done to see if the data follows a normal distribution using tests like Shapiro-Wilk test or Kolmogorov-Smirnov test.

#Since Shapiro-Wilk is suitable for smaller sample size and we are working with larger sample size, oped for Kolmogorov-Smirnov test

ks.test(superpop_data$ScaledTelomericReads, "pnorm", mean=mean(superpop_data$ScaledTelomericReads), sd= sd(superpop_data$ScaledTelomericReads))
```



```{r Non-ParametricANOVAforSuperpopulations }
#Since there are significant differences in telomeric reads among the populations and the assumptions are not met, considered using a non-parametric alternative like the Kruskal-Wallis test.

# Kruskal-Wallis test (non-parametric alternative)
kruskal.test(ScaledTelomericReads ~ Superpopulation.name, data = superpop_data)

#Kruskal-Wallis chi-squared = 96.28, df = 4,
#p-value < 2.2e-16

#The Kruskal-Wallis test result indicates a highly significant difference among the five superpopulations (p-value < 2.2e-16) Since the p-value is much smaller than 0.05, we can reject the null hypothesis, meaning that the distributions of scaled telomeric reads are not the same across the superpopulations

```



```{r Dunn's test with Bonferroni correction}

# Ensuring Superpopulation.name is a factor
...
dunnTest(ScaledTelomericReads ~ Superpopulation.name, 
         data = superpop_data, method = "bonferroni")


```


```{r Among Male and female for each Pop}

by(telo_data$ScaledTelomericReads,
   list(telo_data$Population, telo_data$Gender),
   shapiro.test)

#for each group, p < 0.05 which means the data is not normally distributed. Therefore, proceeding with non-parametric tests.
```


```{r Independent t-test for male and female}


# Example loop to perform Wilcoxon tests across all populations
results <- list()
for (pop in unique(telo_data$Population)) {
  pop_data <- subset(telo_data, Population == pop)
  test_result <- wilcox.test(ScaledTelomericReads ~ Gender, data = pop_data)
...
adjusted_pvalues <- p.adjust(unlist(results), method = "bonferroni")
